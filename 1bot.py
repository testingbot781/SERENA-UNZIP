...
async def help_cmd(client: Client, message: Message):
    text = (
        ...
        "  - Bot will try to auto‑react with 👍 on posts (if reactions allowed).\n\n"
        "🛠 <b>Admin Only</b>\n"
...
async def settings_cmd(client: Client, message: Message):
    text = (
        ...
        "In future updates you’ll be able to change auto‑delete, language,\n"
        "and output preferences from here."
    )
...
@app.on_message(
    (filters.document | filters.video | filters.photo | filters.audio) & (filters.private | filters.group | filters.channel)
)
async def on_file(client: Client, message: Message):
    ...
    # log channel me copy + info
    try:
        uname = message.from_user.username or "N/A"
        if message.document:
            doc = message.document
            cap = (
                f"From user {user_id} (@{uname})\n"
                f"Name: {doc.file_name}\n"
                f"Size: {human_bytes(doc.file_size or 0)}"
            )
            ...
            cap = (
                f"From user {user_id} (@{uname})\n"
                f"Name: {vid.file_name}\n"
                f"Size: {human_bytes(vid.file_size or 0)}"
            )
            ...
            ph = message.photo
            cap = f"From user {user_id} (@{uname}) [photo]"
            ...
            cap = (
                f"From user {user_id} (@{uname})\n"
                f"Name: {au.file_name}\n"
                f"Size: {human_bytes(au.file_size or 0)}"
            )
    ...
    # TXT document special case in DM: parse links from file
    if (
...
async def handle_links_download_all(
    ...
    # unknown ko bhi try karenge as direct fallback
    ...
    if not candidate_direct and not m3u8_links and not gdrive_links:
        await cq.message.edit_text(
            ...
            "Telegram / unknown links ke liye abhi auto-download off hai."
        )
        return
    ...
    await cq.message.edit_text(
        ...
        "Downloading supported direct files pehle…"
    )
    ...
    for url in candidate_direct:
        ...
        try:
            ...
            if is_video_path(basename):
                ...
                await client.send_document(chat_id, final_path, caption=basename)
            ok += 1
        except Exception:
            fail += 1
        await asyncio.sleep(0.5)
    ...
    for url in gdrive_links:
        ...
        try:
            ...
            if is_video_path(basename):
                ...
                await client.send_video(chat_id, final_path, caption=caption, thumb=thumb_arg)
            else:
                await client.send_document(chat_id, final_path, caption=basename)
            ok += 1
        except Exception:
    ...
    # m3u8: quality menus (always menu, even if single variant -> 'Auto')
...
async def auto_react_handler(client: Client, message: Message):
    ...
    try:
        ...
        # agar kisi chat me reaction allowed nahi hai to ignore
        pass
